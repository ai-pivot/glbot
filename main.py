...
def _build_first_prompt(project, mr_iid, mr_summary, author, note_body):
    return (
        ...
        f"**评论:** {note_body}\n\n"
        f"---\n\n"
        f"对这个 MR 进行 Code Review。\n\n"
        f"**你必须使用以下专用工具（不可用 Fetch/Shell/git clone 代替）:**\n"
        f"- `get_mr_changes` → 获取 MR 的完整 diff 和 diff_refs\n"
        f"- `get_mr_info` → 获取 MR 基本信息\n"
        f"- `post_mr_review` → 发表评审结果（整体总结 body + line_comments 行级评论列表）\n"
        f"- `post_mr_note` → 发表普通评论\n\n"
        f"**强制规则:**\n"
        f"- 只评论有实际问题的代码行\n"
        f"- 行级评论标注：🔴 严重 / 🟡 建议\n"
...
def _build_rereview_prompt(project, mr_iid, mr_summary, author, note_body):
    return (
        ...
        f"用户已修改代码，请重新审查。\n\n"
        f"**你必须使用以下专用工具:**\n"
        f"- `get_mr_changes` → 获取最新 diff\n"
        f"- `get_mr_info` → 获取 MR 信息\n"
        f"- `post_mr_review` → 发表评审（body + line_comments）\n\n"
        f"**强制规则:**\n"
        f"- 只评论仍有实际问题的代码行\n"
        f"- 已修复的问题不需要评论\n"
        f"- 行级评论标注：🔴 严重 / 🟡 建议\n"
        f"- 不要尝试 Fetch gitlab.robocraft.dev 或 git clone"
    )
